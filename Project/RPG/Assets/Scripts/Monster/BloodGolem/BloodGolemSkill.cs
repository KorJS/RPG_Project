...
public class BloodGolemSkill : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // 죽음 리턴
        if (monsterState.currentState == TypeData.State.죽음) { return; }

        // 이동중일때 리턴
        if (monsterState.currentState == TypeData.State.이동) { return; }

        // 스킬상태가 아닐때 리턴
        if (monsterState.currentState != TypeData.State.스킬) { return; }
