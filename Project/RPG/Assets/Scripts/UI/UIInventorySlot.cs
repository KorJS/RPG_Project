...
public class UIInventorySlot : MonoBehaviour
{
    ...
    void Awake()
    {
        uiInventory = GameObject.Find("Inventory_Panel").GetComponent<UIInventory>();
    ...
    void OnDragStart()
    {
        ...
        uiInventory.tempIcon = slotSettings.uiIcon;
    ...
    void StopDragging(GameObject obj)
    {
        if (isDragging)
        {
            isDragging = false;
            OnDragDropRelease(obj);
        }
    }

    void OnDragDropRelease(GameObject obj)
    {
        // 드래그중에 복사한 임시 오브젝트 제거
        DestoryTemporary();

        UITexture targetTexture = obj.transform.GetChild(0).GetComponent<UITexture>(); // 타겟 아이콘
        switch (obj.tag)
        {
            case "Inventory":
                break;
        ...
        UIInventorySlot targetSlot = obj.GetComponent<UIInventorySlot>(); // 타겟 정보
    ...
    private GameObject CreateTemporary()
    {
        ...
        else
        {
            ...
            uiInventory.tempDraggingPanel = panelObj;
